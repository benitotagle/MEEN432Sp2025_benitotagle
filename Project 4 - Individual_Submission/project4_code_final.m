...
s = linspace(0, total_length, npts)';  % [m]

% Elevation profile using sigmoid-shaped rise and fall
sigmoid = @(x, x0, w) (exp((x - x0)/w) ./ (1 + exp((x - x0)/w)));
z_max = 10;  % [m] max elevation
zpath = z_max * sigmoid(s, l_st/2, 50);                             % Ascent
zpath = zpath - z_max * sigmoid(s, l_st + l_curve + l_st/2, 50);   % Descent

% Compute slope and slope angle (in radians)
slope = [0; diff(zpath)./diff(s)];     % dz/ds
slope_angle_rad = atan(slope);         % slope angle θ [rad]
